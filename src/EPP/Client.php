...
use AgileGeeks\EPP\EPP_Protocol;
use AgileGeeks\EPP\EPP_Exception;

require_once('Protocol.php');
require_once('Exception.php');

$GLOBALS['EPP_Client_Version'] = '0.0.4';
...
class EPP_Client {
	...
	*/
	...
	function connect($host, $port=700, $timeout=1, $ssl=true, $context=NULL) {
		...
		if (!stream_set_timeout($this->socket,$timeout)) {
			throw new EPP_Exception("Failed to set timeout on socket: $errstr (code $errno)");
		}
		// Set blocking
		if (!stream_set_blocking($this->socket,0)) {
			throw new EPP_Exception("Failed to set blocking on socket: $errstr (code $errno)");
		}
        $frame = $this->getFrame();
        return $frame;
	}
	...
	*/
	function getFrame() {
		return EPP_Protocol::getFrame($this->socket);
	}
	...
	*/
	function sendFrame($xml) {
		return EPP_Protocol::sendFrame($this->socket, $xml);
	}
